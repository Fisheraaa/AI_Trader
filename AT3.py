...
POS_FILE = "my_positions.json"  # Upgraded to JSON
POS_LEGACY_FILE = "my_positions.txt"  # Compatible with old format
KNOWLEDGE_FILE = "knowledge_base.md"
DECISION_LOG_FILE = "data/ai_decision_log.json"

# Dynamic RR levels (fixed version)
VOL_LOW = 15.0
VOL_HIGH = 25.0
RR_LOW_VOL = 1.8
RR_MID_VOL = 2.0
...
# Fusion scoring weights (new)
QUANT_WEIGHT = float(os.getenv("QUANT_WEIGHT", "0.65"))
AI_WEIGHT = float(os.getenv("AI_WEIGHT", "0.35"))
...
class AutoTraderV3:
    ...
    def load_my_positions(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(data, dict):
                return {self.normalize_symbol(k): v for k, v in data.items()}
            return {}
        except Exception:
            return {}
    ...
    def get_market_environment(self) -> Dict[str, Any]:
        ...
        conn = sqlite3.connect(DB_PATH)
        try:
            md = pd.read_sql("SELECT * FROM market_daily ORDER BY date DESC LIMIT 400", conn)
            if md.empty:
                return env
            ...
            for _, row in focus.iterrows():
                ...
                if code == "sh000300" and close < ma20:
                    hs300_below_ma20 = True
            ...
            # 新增：risk_off 开关
            risk_off = bool((breadth <= 40) and (hv >= VOL_HIGH) and hs300_below_ma20)
            ...
            summary += f"- 风险状态: {'RISK_OFF(防守模式)' if risk_off else 'RISK_ON(可进攻)'}"
    ...
    def get_structured_features(self, symbol: str, market_map: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "trend": {
                ...
                "kdj_j": ctx.get("kdj_j"),
                "bull_votes": mtf.get("bull_votes", 0),
            },
            "volume": {
    ...
    # ----------------- Quant Scoring (新增, 不改变原风格输出) -----------------
    def calc_quant_score(self, feat: Dict[str, Any]) -> float:
        score = 50.0
        try:
            price = float(feat.get("price") or 0)
            atr = float(feat.get("atr14") or 0)
            macd = float(feat.get("trend", {}).get("macd_hist") or 0)
            vol_ratio = float(feat.get("volume", {}).get("vol_ratio") or 1)
            ma_rel = feat.get("trend", {}).get("ma480_relation", "below")
            bull_votes = int(feat.get("trend", {}).get("bull_votes") or 0)
    ...
    # ----------------- LLM Calling -----------------
    def call_model_once(self, model: str, prompt: str, temperature: float = LLM_TEMPERATURE) -> Dict[str, str]:
    ...
    def race_call_models_by_tiers(self, prompt: str, temperature=LLM_TEMPERATURE, skip_models: Optional[Set[str]] = None) -> Dict[str, str]:
    ...
    def call_with_quality_priority(self, prompt: str, temperature: float = LLM_TEMPERATURE) -> Dict[str, str]:
    ...
    def parse_instruction_lines(self, text: str) -> List[Dict[str, Any]]:
        ...
        2) 新格式(8列):
        [代码] | [胜率%] | [AI把握0~1] | [动作] | [防守价] | [目标价] | [持仓天数预估] | [失效条件]

        3) 带标题键值格式(推荐):
        代码=sh512690 | 胜率=62% | AI把握=0.64 | 动作=减仓 | 防守价=0.502 | 目标价=0.548 | 持仓天数=3 | 失效条件=收盘跌破防守价
        """
        rows = []
        ...
        for line in text.splitlines():
            ...
            def _to_float(x: str, default=None):
                try:
                    s = re.sub(r"[^0-9.\-]", "", x)
                    if s == "":
                        return default
            ...
            # ---------- 新格式(8列) ----------
            if len(parts) >= 8:
                symbol = self.normalize_symbol(_val(parts[0]).strip("[] "))
                action = _val(parts[3]).strip("[] ").lower()

                win_rate = _to_float(_val(parts[1]), None)
                ai_conf = _to_float(_val(parts[2]), None)
                stop = _to_float(_val(parts[4]), None)
                target_txt = _val(parts[5]).strip()
                days_f = _to_float(_val(parts[6]), None)

                if None in (win_rate, ai_conf, stop, days_f):
                    continue

                if target_txt in ("-", "N/A", "n/a", "无"):
                    target = -1.0
                else:
                    target = _to_float(target_txt, None)
                    if target is None:
                        continue
                ...
                rows.append({
                    ...
                    "ai_confidence": max(0.0, min(1.0, float(ai_conf))),
                    "action": action,
                    "stop": float(stop),
                    "target": float(target),
                    "days": int(float(days_f)),
                    "invalid_if": _val(parts[7]),
                    "raw": line,
                })
                continue
            ...
            if len(parts) >= 6:
                symbol = self.normalize_symbol(_val(parts[0]).strip("[] "))
                action = _val(parts[2]).strip("[] ").lower()

                win_rate = _to_float(_val(parts[1]), None)
                stop = _to_float(_val(parts[3]), None)
                target_txt = _val(parts[4]).strip()
                days_f = _to_float(_val(parts[5]), None)

                if None in (win_rate, stop, days_f):
                    continue
                ...
                rows.append({
                    "symbol": symbol,
                    "win_rate": max(0.0, min(100.0, float(win_rate))),
                    "ai_confidence": max(0.0, min(1.0, float(win_rate) / 100.0)),
                    "action": action,
                    "stop": float(stop),
                    "target": float(target),
                    "days": int(float(days_f)),
    ...
    def apply_risk_gate(
        ...
        for r in rows:
            ...
            if symbol not in market_map:
                r["status"] = "INVALID_NO_MARKET_DATA"
                r["reason"] = "无市场数据"
                validated.append(r)
                continue
            ...
            if symbol in positions:
                ...
                if role == "core" and not (15 <= r["days"] <= 35):
                    r["status"] = "INVALID_CORE_HOLD_DAYS"
                    r["reason"] = "底仓周期违反约束(15-35天)"
                    validated.append(r)
                    continue
                if role == "attack" and not (3 <= r["days"] <= 5):
                    r["status"] = "INVALID_ATTACK_HOLD_DAYS"
                    r["reason"] = "攻击仓周期违反约束(3-5天)"
                    validated.append(r)
                    continue

            # 新增：risk_off 时禁止新增仓位
            if risk_off and symbol not in positions and action in ("买入", "开仓", "加仓", "持股", "持有"):
                r["status"] = "INVALID_RISK_OFF"
                r["reason"] = "当前为防守模式，禁止新增仓位"
                r["action"] = "观望"
                validated.append(r)
                continue
            ...
            if symbol not in positions and action in ("买入", "开仓", "加仓", "持股", "持有"):
                ...
                if rr < rr_new_threshold:
                    r["status"] = "INVALID_RR_NEW_TOO_LOW"
            ...
            # 加仓
                ...
                if rr_add < rr_add_threshold:
                    r["status"] = "INVALID_RR_ADD_TOO_LOW"
    ...
    def fuse_scores(self, rows: List[Dict[str, Any]], quant_scores: Dict[str, float]) -> List[Dict[str, Any]]:
        out = []
        for r in rows:
            q = float(quant_scores.get(r["symbol"], 50.0))
            ai_score = float(r.get("ai_confidence", 0.5)) * 100.0
            final_score = round(QUANT_WEIGHT * q + AI_WEIGHT * ai_score, 2)

            x = dict(r)
            x["quant_score"] = round(q, 2)
            x["final_score"] = final_score
            x["confidence_grade"] = "A" if x.get("win_rate", 0) >= 70 else ("B" if x.get("win_rate", 0) >= 55 else "C")
            out.append(x)

        out = sorted(out, key=lambda z: (-z["final_score"], z["symbol"]))
        return out
    ...
    def build_prompt(
        ...
        return (
            ...
            "2) 新���仓必须满足 RR=(Target-Price)/(Price-Stop) >= 动态门槛。\n"
            "3) 加仓必须满足 RR_add=(NewTarget-Price)/(Price-TrailingStop) >= 动态门槛。\n"
            "4) 防守价优先采用 ATR 止损逻辑（多头：当前价-2*ATR）。\n"
            "5) 输出必须包含明确动作，不得模糊。\n"
            f"6) 当前风险状态 risk_off={str(risk_off).lower()}；若 risk_off=true，禁止新增仓位（候选只能观望）。\n\n"
            "【知识库（最高优先级）】\n"
            f"{knowledge_text}\n\n"
            "步骤1：环境判断\n"
            ...
            "A. 【波段防御区】\n"
            "B. 【短线进攻区】\n"
            "C. 【实战指令】（每行必须带字段名，便于直接执行）\n"
            "格式:\n"
            "代码=sh512690 | 胜率=62% | AI把握=0.64 | 动作=减仓 | 防守价=0.502 | 目标价=0.548 | 持仓天数=3 | 失效条件=收盘跌破防守价且量能放大\n"
            "若清仓/观望无目标价，写: 目标价=- ; 持仓天数=0\n"
            "D. 【今日避雷笔记】\n"
    ...
    def run_engine(self):
        ...
        # 结构化特征：保持“先持仓后候选”
        pos_symbols = sorted([s for s in market_map.keys() if s in positions])
        cand_symbols = sorted([s for s in market_map.keys() if s not in positions])[:MAX_CANDIDATES]
        ...
        # 新增 quant_score（不改变原输出主风格）
        quant_scores: Dict[str, float] = {}
        for feat in position_features + candidate_features:
            quant_scores[feat["symbol"]] = self.calc_quant_score(feat)
        ...
        prompt = self.build_prompt(
            ...
            risk_off=market_env.get("risk_off", False),
        )

        res = self.call_with_quality_priority(prompt, temperature=LLM_TEMPERATURE)
        ...
        checked_rows = self.fuse_scores(checked_rows, quant_scores)
        ...
        if not checked_rows:
            ...
            for r in checked_rows:
                ...
                reason = f" | 原因:{r.get('reason','-')}" if str(r.get("status", "")).startswith("INVALID") else ""
                gate_lines.append(
                    f"- {r['symbol']} | 动作:{r['action']} | 状态:{r.get('status','UNKNOWN')}"
                    f" | 胜率:{r.get('win_rate', 0)}% | AI把握:{round(float(r.get('ai_confidence',0)),3)}"
                    f" | Quant:{r.get('quant_score', 0)} | Final:{r.get('final_score', 0)} | 等级:{r.get('confidence_grade','-')}"
                    f"{rr_txt}{reason}"
                )
